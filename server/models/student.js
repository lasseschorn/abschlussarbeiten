...
function(connection, callback) {
    return {
        getById: (req) => {
          ...
          var sql = 'select gradID,matrikelnummer as bezeichnung, personenID, vorname, nachname, geschlecht, email, studiengangID, studiengang, kategorieID, kategorie, akademischergrad from `akademischer Grad` a inner join ( select personenID as personenID, vorname as vorname, nachname as nachname, geschlecht as geschlecht, `e-mail` as email, studiengangID as studiengangID, sBezeichnung as studiengang, kategorie.kategorieID as kategorieID, kategorie.bezeichnung as kategorie, `akademischer grad_gradID` as akademischergrad from kategorie inner join ( select personenID, vorname, nachname, geschlecht, `e-mail`, studiengang.StudiengangID, studiengang.bezeichnung as sBezeichnung,abschlussarbeiten_kategorie_kategorieID,`akademischer grad_gradID`, matrikelnummer from studiengang inner join ( select * from person p inner join student s on p.personenID = s.person_personenID) d on studiengang.studiengangID = d.abschlussarbeiten_studiengang_studiengangID ) g on kategorie.kategorieID = g.abschlussarbeiten_kategorie_kategorieID ) ka on a.gradID = ka.`akademischer grad_gradID` where personenID = ?'
